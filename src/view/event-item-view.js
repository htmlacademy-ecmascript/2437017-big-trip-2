...
function createEventItemTemplate(point, destinations, offers) {
  ...
  // Находим соотвествующий текущиму point destination
  ...
  // Находим соотвествующий текущиму point offers
  ...
  console.log(`${dateEnd} ${dateStart} ${durationTime}`);
  ...
  // const typeOffers = offers.find((off) => off.type === point.type).offers; /* тип из mockOffers который соответсвует типу из mockPoints */
  // const pointOffers = typeOffers.filter((typeOffers) => point.offers.includes(typeOffers.id)); /* массив для данной контретной точки */
